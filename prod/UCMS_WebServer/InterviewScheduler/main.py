...
def main():
    ...
    tmp_filename = output_filename + ".tmp"  # ✅ 임시 파일 경로

    if not os.path.exists(input_filename):
        print(f"File not found: {input_filename}")
        return

    # ✅ 디렉토리 생성을 solve 이전으로 이동 (크래시 시 디렉토리 누락 방지)
    ...
    # ✅ 임시 파일에 먼저 쓴 뒤, 완성되면 원자적으로 rename
    with open(tmp_filename, 'w', encoding='utf-8') as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
    os.replace(tmp_filename, output_filename)  # 원자적 교체 (읽기 도중 덮어쓰기 방지)
