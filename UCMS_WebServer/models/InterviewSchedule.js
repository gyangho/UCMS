...
class InterviewSchedule {
  static createInterviewSchedule = async (planId, scheduleData) => {
    ...
    for (const interviewerId of interviewers) {
      ...
      await db.query(
        `INSERT INTO interview_schedules (plan_id, interview_date, time_slot, interviewer_id, interviewee_id) VALUES (?, ?, ?, ?, ?)`,
        [
          planId,
          interviewDate,
          timeSlot,
      ...
      const [insertedRows] = await db.query(
        `SELECT * FROM interview_schedules WHERE plan_id = ? AND interview_date = ? AND time_slot = ? AND interviewer_id = ? AND interviewee_id = ? ORDER BY id DESC LIMIT 1`,
        [
          planId,
          interviewDate,
          timeSlot,
