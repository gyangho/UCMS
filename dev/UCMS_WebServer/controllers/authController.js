...
class AuthController {
  ...
  static getRedirectUri(req) {
    return `${req.protocol}://${req.get("host")}/auth/redirect`;
  }
  ...
  static async authorize(req, res) {
    ...
    authorizeUrl.searchParams.set("client_id", process.env.KAKAO_REST_KEY);
  ...
  static async redirect(req, res) {
    try {
      ...
      console.log("User login - ID: " + id + " name: " + nickname);
      ...
      if (!user) {
        ...
        if (!authority || !user.chat_room_id) {
          ...
          await User.update(user.id, nickname, profile_image, thumbnail_image);
  ...
  static async regenerateCode(req, res) {
    try {
      ...
      await PendingAuth.updateAuthCode(req.session.pendingAuth, newAuthCode);
  ...
  static async cancelAuth(req, res) {
    console.log(req.session.pendingAuth);
    await PendingAuth.deleteByAuthCode(req.session.pendingAuth);
  ...
  static async confirmMember(req, res) {
    try {
      ...
      await Member.updateUserId(selectedMember.id, req.session.userId);
      // 선택한 멤버의 권한 수정
