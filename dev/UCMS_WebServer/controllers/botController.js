...
class BotController {
    ...
    static async publicResponse(
        ...
        switch (content) {
            ...
            case "일정":
                const allEvents = await Event.findByAuthorityWithoutHolidays(authority);
                const now = new Date();
                ...
                message = `일정 목록🎉\n${events
                    .map((event) => {
                        ...
                        const startTime = formatBtn(event.start);
                        const endTime = formatBtn(event.end);
