...
export function RecruitInstanceDetailPage({ path }: { path: string }) {
  ...
  // 2026-09-03: Load linked responses in every lifecycle state so the overall rating ratio is always accurate.
  const load = useCallback(async () => { setLoading(true); try { const data = await requestData<{ instance: RecruitmentInstance }>(`/api/recruit/instances/${id}`); setItem(data.instance); setIsEditing(data.instance.status === "draft"); setGeneration((current) => current || inferGeneration(data.instance.title)); setForm({ title: data.instance.title, recruitStart: toLocalInput(data.instance.recruitStart), recruitEnd: toLocalInput(data.instance.recruitEnd), interviewStart: toLocalInput(data.instance.interviewStart), interviewEnd: toLocalInput(data.instance.interviewEnd), formUrl: data.instance.formUrl ?? "", promotionCopy: data.instance.promotionCopy ?? "" }); setResponses([]); if (data.instance.formId) { const responseData = await requestData<{ responses: RecruitResponseRow[] }>("/api/recruit/responses"); setResponses((responseData.responses ?? []).filter((row) => row.formId === data.instance.formId)); } } catch (e) { setMessage(e instanceof Error ? e.message : "모집 상세를 불러오지 못했습니다."); } finally { setLoading(false); } }, [id]);
  useEffect(() => { load(); }, [load]);
  ...
  async function deleteRecruitment() {
    ...
    try {
      ...
      setMessage(e instanceof Error ? e.message : "모집을 삭제하지 못했습니다.");
      setIsDeleting(false);
      setBusyAction(null);
    }
  }
  ...
  return <section className="stack-page recruit-page">
    ...
    {/* 2026-09-03: Keep the overall rating ratio visible from draft through closed recruitment. */}
    <RecruitMetrics item={item} rows={responses} />{item.status !== "draft" ? <ApplicantTable rows={responses} /> : null}
  </section>;
}

// 2026-09-03: Show gender and every evaluation state as one visual snapshot in every recruitment phase.
function RecruitMetrics({ item, rows }: { item: RecruitmentInstance; rows: RecruitResponseRow[] }) {
  ...
  return <section className="recruit-metrics-grid"><article className="data-card"><h2>지원 현황</h2><p>지원자 {item.applicantCount}명</p>{total ? <div className="gender-ratio" aria-label={`남성 ${maleRate}%, 여성 ${femaleRate}%`}><span className="male" style={{ width: `${maleRate}%` }}>남 {maleRate}%</span><span className="female" style={{ width: `${femaleRate}%` }}>여 {femaleRate}%</span></div> : <p className="muted-copy">성별 집계 데이터가 없습니다.</p>}</article><RatingPie counts={counts} total={rows.length} /></section>;
}
