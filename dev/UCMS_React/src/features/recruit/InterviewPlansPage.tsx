  ...
  return (
    <section className="stack-page interview-page">
      <div className="interview-hero compact-hero">
        ...
        <button className="hero-button" type="button" onClick={() => navigate("/recruit/interview/new")}>새 계획 만들기</button>
      </div>
        ...
        <div className="interview-plan-grid">
          {plans.map((plan) => (
            ...
            >
              ...
              <div className="plan-card-footer"><span>담당 {plan.owner ?? "-"}</span><strong>상세 보기 →</strong></div>
            </article>
          ))}
        </div>
      )}
    ...
    <section className="interview-wizard">
      ...
      {step > 0 ? (
        <div className="wizard-actions">
          ...
          {step < 3 ? <button type="button" onClick={next}>다음 단계</button> : <button disabled={isLoading} type="button" onClick={createPlan}>{isLoading ? "생성 중..." : "면접 계획 생성"}</button>}
        </div>
      ) : null}
    </section>
  );
...
export function InterviewPlanDetailPage({ path }: { path: string }) {
  ...
  const loadPlan = useCallback(async () => {
    ...
    try {
      ...
      setPlan(data.plan); setDates(data.interviewDates ?? []); setInterviewers(data.interviewers ?? []);
  ...
  async function generateTimetable() {
    if (!window.confirm(`최소 ${plan?.panelSize ?? 2}명의 면접관으로 스케줄을 생성하시겠습니까?`)) return;
    setMessage(null);
    setActionError(null);
    ...
    try {
      const data = await requestData<{
        ...
        method: "POST",
        body: JSON.stringify({ minInterviewers: plan?.panelSize ?? 2 }),
      });
      setMessage(
        data.isPerfect
  ...
  async function deletePlan() {
    ...
    navigate("/recruit/interview/plans");
  }
  ...
  return (
    <section className="stack-page interview-detail-page">
      <div className="page-heading plan-detail-heading">
        ...
        <div className="toolbar">
          {plan.status === "draft" ? <><button className="success-button" disabled={isGenerating || isChangingStatus} type="button" onClick={generateTimetable}>{isGenerating ? "스케줄 생성 중..." : "면접 스케줄 생성"}</button><button disabled={isGenerating || isChangingStatus} type="button" onClick={() => changeStatus("active")}>확정</button></> : null}
          {plan.status === "active" ? <><button className="secondary-button" disabled={isChangingStatus} type="button" onClick={() => changeStatus("draft")}>확정 취소</button><button disabled={isChangingStatus} type="button" onClick={() => changeStatus("completed")}>면접 종료</button></> : null}
      ...
      <DetailSection title={`면접 스케줄 (${schedule.length}건)`}>{schedule.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>면접 날짜</th><th>시간대</th><th>면접관</th><th>피면접자</th></tr></thead><tbody>{schedule.map((item) => <tr key={item.id}><td>{item.start}</td><td>{item.end}</td><td>{(item.interviewerNames ?? []).join(", ") || "-"}</td><td>{item.applicantName ?? "-"}</td></tr>)}</tbody></table></div> : <EmptyState title="생성된 면접 스케줄이 없습니다." />}</DetailSection>
    </section>
  );
}
