...
require("dotenv-expand").expand(
  require("dotenv").config({
    path:
      process.env.NODE_ENV === "dev" ? "./keys/.env.dev" : "./keys/.env.prod",
  }),
);

// 2026-07-24: Use the shared infrastructure port while preserving PORT for direct local execution.
const PORT = Number(process.env.SHAREDB_PORT ?? 3001);
if (!Number.isInteger(PORT) || PORT < 1 || PORT > 65535) {
  throw new Error(
    "SHAREDB_PORT or PORT must be an integer between 1 and 65535.",
  );
}
...
console.log(`🚀 공유문서 WebSocket 서버가 포트 ${PORT}에서 실행 중입니다.`);
...
async function handleLockLine(clientId, message) {
  ...
  try {
    console.log(`🔒 줄 락 요청: ${docId} 줄 ${lineNumber} by ${clientId}`);
    ...
    broadcastLockStatus(docId, formId, lineNumber, clientId, "locked");

    console.log(`✅ 줄 락 성공: ${docId} 줄 ${lineNumber} by ${clientId}`);
  } catch (error) {
...
async function handleUnlockLine(clientId, message) {
  ...
  try {
    console.log(`🔓 줄 락 해제 요청: ${docId} 줄 ${lineNumber} by ${clientId}`);
    ...
    if (existingLock && existingLock.clientId === clientId) {
      ...
      // 다른 클라이언트들에게 락 상태 브로드캐스트
      broadcastLockStatus(docId, formId, lineNumber, null, "unlocked");
...
function broadcastLockStatus(docId, formId, lineNumber, clientId, status) {
  const message = {
    type: "lock-status",
    docId: docId,
    lineNumber: lineNumber,
  ...
  console.log(`📡 락 상태 브로드캐스트: ${broadcastCount}개 클라이언트`);
}
...
function releaseClientLocks(clientId) {
  lineLocks.forEach((docLocks, docId) => {
    docLocks.forEach((lockInfo, lineNumber) => {
      ...
      const session = clientSessions.get(clientId);
      broadcastLockStatus(docId, session?.formId, lineNumber, null, "unlocked");
    });
    if (docLocks.size === 0) {
  ...
  lineLocks.forEach((docLocks, docId) => {
    docLocks.forEach((lockInfo, lineNumber) => {
      if (lockInfo.expiresAt < now) {
        console.log(`⏰ 만료된 락 자동 해제: ${docId} 줄 ${lineNumber}`);
        docLocks.delete(lineNumber);
      }
    });
  });
